...
class UserSession:
    ...
    def _build_sharegpt_history(self):

        def gen_dummy_text(length):
            return ' '.join(["hi"] * length)

        # 560 is the overall average tokens of (input + output) across all entries in sharegpt dataset
        dummy_text = gen_dummy_text(560 * self.question_id)
        return f"{self.user_config.user_id}:{dummy_text}. "
    ...
    def _launch_new_request(self, timestamp: float,
        ...
        prompt = ''
        if self.use_sharegpt and self.question_id > 0 and len(self.chat_history) == 0:
            prompt = self._build_sharegpt_history() + prompt
        if self.use_sharegpt:
        ...
        if self.use_sharegpt:
            max_tokens = None
            if self.start_with_gpt:
                conversation = self.sharegpt_data['conversations'][2 * self.question_id]
                if 'num_tokens' in conversation:
                    max_tokens = conversation['num_tokens']
            else:
                conversation = self.sharegpt_data['conversations'][2 * self.question_id - 1]
                if 'num_tokens' in conversation:
                    max_tokens = conversation['num_tokens']
            if max_tokens:        
                ...
                max_tokens = 280 # set to average sharegpt output token num
        else:
    ...
    def set_internal_state(self, offset: float, timestamp: float):
        ...
        num_passed_questions = \
                int(offset / self.user_config.gap_between_requests)
...
class UserSessionManager:
    ...
    def __init__(self,
        ...
        self.gap_between_users = gap_between_requests_per_user * workload_config.num_rounds / (
